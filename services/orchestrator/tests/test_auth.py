"""Tests for inbound API-key authentication. This service sits in front of
all five downstream services, so leaving it unauthenticated is strictly
worse than leaving any single one open."""
...
def test_auth_disabled_by_default_allows_any_request(monkeypatch):
    monkeypatch.delenv("ORCHESTRATOR_REQUIRE_API_KEY", raising=False)
    get_settings.cache_clear()
    verify_api_key(x_api_key=None)


def test_auth_required_rejects_missing_header(monkeypatch):
    monkeypatch.setenv("ORCHESTRATOR_REQUIRE_API_KEY", "1")
    monkeypatch.setenv("ORCHESTRATOR_API_KEYS", "secret-key-one")
    get_settings.cache_clear()
...
def test_auth_accepts_any_of_multiple_configured_keys(monkeypatch):
    monkeypatch.setenv("ORCHESTRATOR_REQUIRE_API_KEY", "1")
    monkeypatch.setenv("ORCHESTRATOR_API_KEYS", "key-a,key-b, key-c ")
    get_settings.cache_clear()
...
def test_auth_required_but_misconfigured_fails_closed(monkeypatch):
    monkeypatch.setenv("ORCHESTRATOR_REQUIRE_API_KEY", "1")
    monkeypatch.setenv("ORCHESTRATOR_API_KEYS", "")
    get_settings.cache_clear()
