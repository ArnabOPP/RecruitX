...
real HTTP API, hitting all five actual running services (cv-parser,
interview-qa, speech-io, answer-grading, code-eval) and a real Redis —
nothing mocked anywhere in this file. This is the equivalent of every
other service's "live" test file, scaled up to an integration across the
whole platform.

Skips cleanly (with a clear reason) if the full stack isn't running —
this is meant to be run deliberately, with all six services (plus Redis)
started, not as part of the default fast test loop.
"""
...
pytestmark = pytest.mark.skipif(
    ...
    reason="Not all five downstream services + Redis are reachable — start the full stack to run this test.",
)
