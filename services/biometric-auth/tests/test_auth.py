"""Tests for inbound API-key authentication — protects both compute
resources (face detection/embedding inference) and, more importantly here,
the enrolled biometric reference data itself."""
...
def test_auth_disabled_by_default_allows_any_request(monkeypatch):
    monkeypatch.delenv("BIOMETRIC_AUTH_REQUIRE_API_KEY", raising=False)
    get_settings.cache_clear()
    verify_api_key(x_api_key=None)  # must not raise


def test_auth_required_rejects_missing_header(monkeypatch):
    monkeypatch.setenv("BIOMETRIC_AUTH_REQUIRE_API_KEY", "1")
    monkeypatch.setenv("BIOMETRIC_AUTH_API_KEYS", "secret-key-one")
    get_settings.cache_clear()
...
def test_auth_accepts_any_of_multiple_configured_keys(monkeypatch):
    monkeypatch.setenv("BIOMETRIC_AUTH_REQUIRE_API_KEY", "1")
    monkeypatch.setenv("BIOMETRIC_AUTH_API_KEYS", "key-a,key-b, key-c ")
    get_settings.cache_clear()
...
def test_auth_required_but_misconfigured_fails_closed(monkeypatch):
    monkeypatch.setenv("BIOMETRIC_AUTH_REQUIRE_API_KEY", "1")
    monkeypatch.setenv("BIOMETRIC_AUTH_API_KEYS", "")
    get_settings.cache_clear()
