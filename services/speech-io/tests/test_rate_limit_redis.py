...
separate processes — the entire point of SPEECH_IO_RATE_LIMIT_STORAGE_URI.
Ported from interview-qa's tests/test_rate_limit_redis.py — the underlying
`limits` library mechanics are identical regardless of which service uses
them.
...
def redis_uri():
    ...
    port = _free_port()
    container_name = "speech-io-test-redis-ratelimit"
